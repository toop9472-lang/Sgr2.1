...
const AIChatModal = ({ isOpen, onClose, user }) => {
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: 'مرحباً! أنا مساعد صقر الذكي. كيف يمكنني خدمتك اليوم؟\n\nيمكنني مساعدتك في:\n• معرفة رصيد جواهرك\n• كيفية كسب جواهر صقر من الإعلانات\n• نشر مقاطع الريلز (15 ثانية)\n• الدردشة العامة 24/7\n• طرق استبدال الجواهر بالمال الحقيقي'
    }
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const userContext = user
        ? `المستخدم لديه ${user.saqr_gems || user.points || 0} جوهرة. اسمه: ${user.name || 'مستخدم'}.`
        : '';
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          system_message: `أنت المساعد الذكي الرسمي لتطبيق "صقر" — منصة عربية لمشاهدة إعلانات Google AdMob، نشر مقاطع ريلز قصيرة، والدردشة العامة. ${userContext}

قواعد الرد:
- أجب بالعربية الفصحى الواضحة دائماً.
- ردودك قصيرة، احترافية، ومنظمة في نقاط عند الحاجة.
- لا تختلق معلومات؛ إن لم تكن متأكداً اطلب من المستخدم التواصل مع الدعم.
- كن إيجابياً ومشجعاً.

معلومات التطبيق الدقيقة:
- العملة الوحيدة داخل التطبيق: "جواهر صقر" (ليست نقاط).
- كل إعلان Google AdMob يُكمله المستخدم يمنحه 5 جواهر صقر (مكافأة ثابتة).
- استبدال الجواهر بالمال: 500 جوهرة = 3 ريال سعودي.
- مقاطع الريلز: حد أقصى 15 ثانية لكل مقطع، مع إعجابات وتعليقات ومتابعة.
- الدردشة العامة: مجانية بالكامل وتعمل 24/7.
- إعلانات AdMob متاحة على تطبيق الموبايل (iOS/Android). على الويب يمكن للمعلنين الاشتراك كـ"معلن" لرفع إعلاناتهم.

ميزات يحتاج المستخدم معرفتها:
- صفحة "مشاهدة": يكسب فيها الجواهر من إعلانات AdMob.
- صفحة "ريلز": ينشر فيها مقاطع 15 ثانية ويتفاعل.
- صفحة "أعلن": للتقديم كمعلن.
- صفحة "حسابي": عرض الرصيد، السحب، الإعدادات.
- صفحة "ثروات صقر": فرص ربح إضافية بالجواهر.

أجب بأسلوب مباشر ومفيد دون حشو.`
        })
      });
