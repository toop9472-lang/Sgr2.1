...
const SYSTEM_CONTEXT = `أنت المساعد الذكي الرسمي لتطبيق صقر.

قواعد الرد:
1) أسلوبك احترافي وودود وواضح.
2) ردودك قصيرة ومنظمة في نقاط عند الحاجة.
3) إذا كانت الرسالة بسيطة جداً (مثل: مرحباً، شكراً، تمام) رد بشكل طبيعي ولطيف ثم اعرض المساعدة.
4) لا تختلق معلومات غير مؤكدة.

معلومات التطبيق:
- جواهر صقر: للسحب المالي فقط (500 جوهرة = 1 ريال سعودي).
- الألماس: للاستخدام داخل التطبيق (ألعاب/متاجر/ميزات).
- متجر الألماس: حزم رسمية من 3 إلى 19 ريال.
- متجر المميزات: إطارات الملف، إطارات الدردشة، أفاتار، ثيمات، وباقات خاصة.
- الدردشة العامة تستهلك ألماس لكل رسالة.
...
const AIChatModal = ({ visible, onClose }) => {
  ...
  const getOfflineResponse = (question) => {
    ...
    if (q.includes('مرحبا') || q.includes('هلا') || q.includes('السلام') || q.includes('hello') || q.includes('hi')) {
      return 'أهلاً وسهلاً بك!\nأنا جاهز لمساعدتك.\nهل تريد مساعدة في الجواهر، المتجر، الألعاب، أو الحساب؟';
    }

    if (q.includes('شكرا') || q.includes('يسلم') || q.includes('thanks')) {
      return 'على الرحب والسعة! إذا تحب، أقدر أعطيك أفضل خطة سريعة لزيادة الجواهر والألماس يومياً.';
    }
    ...
    if (q.includes('ألماس') || q.includes('diamond')) {
      return 'الألماس هو عملة الاستخدام داخل التطبيق:\n\n• يستخدم في الألعاب والدردشة والمتاجر\n• متجر الألماس يحتوي على حزم من 3 إلى 19 ريال\n• لا يمكن سحب الألماس كمال مباشر\n\nإذا تريد أساعدك تختار أفضل باقة حسب استخدامك.';
    }
