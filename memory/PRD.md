...
## LATEST UPDATE (March 2026) - Version 7.2.0

### تحديث 1 مارس 2026 - نظام لوحات المتصدرين والمتجر والتخزين المؤقت

#### 1. نظام لوحات المتصدرين (Leaderboards)
**الملفات:** `/backend/routes/leaderboards_routes.py`, `/frontend/src/components/GameLeaderboard.jsx`

**الميزات:**
- لوحة متصدرين منفصلة لكل من الـ 12 لعبة
- فلاتر الفترة: يومي، أسبوعي، شهري، كل الأوقات
- جوائز لأفضل 10 لاعبين (نقاط صقر + ألماسات)
- عرض ترتيب المستخدم الحالي
- إرسال النتائج تلقائياً بعد انتهاء كل لعبة

**الـ APIs:**
- `GET /api/leaderboards/game/{game_id}` - لوحة لعبة محددة
- `GET /api/leaderboards/all-games` - جميع اللوحات
- `POST /api/leaderboards/submit-score` - إرسال نتيجة
- `GET /api/leaderboards/my-stats` - إحصائياتي
- `POST /api/leaderboards/claim-reward` - استلام المكافأة

#### 2. نظام المشتريات داخل التطبيق (IAP)
**الملفات:** `/backend/routes/iap_routes.py`, `/frontend/src/components/IAPShop.jsx`

**باقات الألماسات:**
| الباقة | الألماسات | البونص | السعر (ر.س) |
|--------|-----------|--------|-------------|
| البداية | 100 | 0 | 3 |
| الفضية | 250 | +25 | 7 |
| الذهبية | 500 | +75 | 12 |
| البلاتينية | 1000 | +200 | 19 |
| الملوك | 2500 | +750 | 45 |
| الأساطير | 5000 | +2000 | 85 |

**اشتراكات VIP:**
| الخطة | المدة | المميزات | السعر |
|-------|-------|----------|-------|
| أسبوعي | 7 أيام | 50 ألماسة/يوم، بدون إعلانات | 9.99 ر.س |
| شهري | 30 يوم | 100 ألماسة/يوم، مكافآت مضاعفة | 29.99 ر.س |
| سنوي | 365 يوم | 200 ألماسة/يوم، لقب خاص | 249.99 ر.س |

**الـ APIs:**
- `GET /api/iap/products` - جميع المنتجات
- `GET /api/iap/diamond-packages` - باقات الألماس
- `GET /api/iap/vip-plans` - خطط VIP
- `POST /api/iap/purchase` - شراء منتج
- `POST /api/iap/subscribe` - الاشتراك في VIP
- `GET /api/iap/vip-status` - حالة VIP
- `GET /api/iap/my-purchases` - سجل مشترياتي

#### 3. نظام التخزين المؤقت (Cache)
**الملف:** `/backend/routes/cache_routes.py`

- تخزين مؤقت في الذاكرة (In-Memory)
- TTL مخصص لكل نوع بيانات
- إحصائيات استخدام الذاكرة
- APIs للإدارة: `/api/cache/stats`, `/api/cache/clear`

#### حالة الاختبار:
- ✅ Backend: 23/23 اختبارات ناجحة (100%)
- ✅ Frontend: المتجر ولوحات المتصدرين تعمل
- ✅ تم إصلاح bug في البحث عن المستخدم

---

## PREVIOUS UPDATE (March 2026) - Version 7.1.0
