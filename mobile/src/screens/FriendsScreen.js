  ...
  return (
    <View style={styles.friendCard}>
      ...
      <View style={styles.friendInfo}>
        ...
        <Text style={styles.friendSince}>صديق منذ {new Date(friend.friendship_date).toLocaleDateString('ar-SA')}</Text>
      </View>

      <View style={styles.friendActions}>
        <TouchableOpacity style={styles.actionBtn} onPress={() => onMessage(friend)}>
          <Ionicons name="mail" size={18} color="#3b82f6" />
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionBtn} onPress={() => onInvite(friend)}>
          <Ionicons name="game-controller" size={18} color="#22c55e" />
        </TouchableOpacity>
        <TouchableOpacity style={styles.actionBtn} onPress={() => onRemove(friend)}>
  ...
  return (
    <View style={styles.requestCard}>
      <View style={styles.requestAvatar}>
        <LinearGradient colors={['#f59e0b', '#ef4444']} style={styles.avatarPlaceholder}>
          <Text style={styles.avatarText}>{request.from_user_name?.charAt(0) || '?'}</Text>
        </LinearGradient>
      </View>
  ...
  return (
    <View style={styles.searchResultCard}>
      <View style={styles.searchAvatar}>
        <LinearGradient colors={['#06b6d4', '#3b82f6']} style={styles.avatarPlaceholder}>
      ...
      <TouchableOpacity 
        ...
        disabled={pending}
      >
        <Ionicons name={pending ? "hourglass" : "person-add"} size={16} color="#FFF" />
...
const FriendsScreen = ({ user, onClose, onOpenMessages, onOpenGameInvite }) => {
  ...
  const loadData = async (isPullToRefresh = false) => {
    ...
    try {
      ...
      // Load requests
      const requestsRes = await api.fetch(`/api/social/friends/requests/${user?.id}`);
      if (requestsRes.ok) {
  ...
  const searchUsers = async () => {
    ...
    setSearching(true);
    try {
      const res = await api.fetch(`/api/social/users/search?query=${encodeURIComponent(searchQuery)}`);
      if (res.ok) {
    ...
    Alert.alert(
      ...
      [
        { text: 'إلغاء', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const res = await api.fetch(`/api/social/friends/remove/${user?.id}/${friend.id}`, {
                method: 'DELETE'
              });
              if (res.ok) {
                loadData();
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <FriendCard
                ...
                onInvite={(f) => onOpenGameInvite && onOpenGameInvite(f)}
              />
            )}
            ...
            ListHeaderComponent={() => (
              <>
                {requests.incoming.length > 0 && (
                  <Text style={styles.sectionHeader}>طلبات واردة ({requests.incoming.length})</Text>
                )}
              </>
            )}
          />
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <SearchResultCard
                    ...
                    pending={requests.outgoing.find(r => r.to_user_id === item.id)}
                  />
                )}
                contentContainerStyle={styles.listContent}
                showsVerticalScrollIndicator={false}
...
const styles = StyleSheet.create({
  ...
  searchInput: { flex: 1, color: '#FFF', fontSize: 14, paddingVertical: 10, marginHorizontal: 10 },
  searchResultCard: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: 'rgba(255,255,255,0.05)',
